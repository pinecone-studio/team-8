...
export const allowedAdminRoles = new Set(["hr_admin", "finance_manager"]);
...
export function isAllowedAdminRole(role: string | null | undefined): boolean {
  return typeof role === "string" && allowedAdminRoles.has(role);
}
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        token.role = typeof user.role === "string" ? user.role : "hr_admin";
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.role = typeof token.role === "string" ? token.role : "hr_admin";
      }
