...
export default function RuleConfiguration() {
  ...
  const [proposalMode, setProposalMode] = useState<ProposalFormMode | null>(
    null,
  );
  ...
  const { data: benefitsData, loading: benefitsLoading } =
    useGetAdminBenefitsQuery({ skip: !isHr });
  ...
  const refetchQueries = selectedBenefitId
    ? [
        {
          query: GetEligibilityRulesDocument,
          variables: { benefitId: selectedBenefitId },
        },
        {
          query: GetRuleProposalsDocument,
          variables: { benefitId: selectedBenefitId },
        },
      ]
    : [];
  ...
  function fillDemoReject() {
    setRejectReason(
      "This rule conflicts with current eligibility policy. Please revise and resubmit.",
    );
  }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      setActionError(
        e instanceof Error ? e.message : "Failed to approve proposal",
      );
    }
  }
  ...
  async function handleReject() {
    ...
    try {
      ...
      setActionError(
        e instanceof Error ? e.message : "Failed to reject proposal",
      );
    }
  }
      ...
      <main className="flex-1 px-8 py-9">
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-6 py-8 text-center max-w-md">
          ...
          <p className="mt-1 text-xs text-amber-700">
            Rule Configuration is restricted to HR administrators.
          </p>
        </div>
      </main>
    );
          ...
          {benefitsLoading ? (
            ...
            <>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Propose and approve eligibility rule changes for benefits. All
                changes require a second HR admin to approve.
              </p>
            </>
          )}
        </div>
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-5">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <span className="text-xs text-amber-700">
                    — requires a second HR admin to approve
                  </span>
                </div>
                {proposalsLoading ? (
                          ...
                          <div>
                            ...
                            {p.proposedByEmployeeId === me?.id && (
                              <p className="mt-1 text-[11px] font-medium text-amber-700">
                                ⚠ You proposed this — another HR admin must
                                approve it
                              </p>
                            )}
                          </div>
                ...
                >
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    <button
                      ...
                      className="rounded-xl border bg-yellow-400 border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
                    >
                      Fill demo
                    </button>
                    <button
                ...
                >
                  ...
                  {proposalMode === "delete" ? (
                    <div className="rounded-lg border border-red-200 bg-white px-4 py-3 text-sm text-slate-700">
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        This will require second-admin approval before taking
                        effect.
                      </p>
                    </div>
                  ) : (
                  ...
                  <div className="mt-3 flex items-center justify-between gap-2">
                    <p className="text-xs text-slate-500">
                      A second HR admin will need to approve this before it
                      takes effect.
                    </p>
                    ...
                    >
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        {[
                          ...
                          <div key={j}>
                            <div
                              className={`h-2.5 ${lw} rounded-full bg-slate-200/80 animate-pulse`}
                            />
                            <div
                              className={`mt-1.5 h-3.5 ${vw} rounded-full bg-slate-200/80 animate-pulse`}
                            />
                          </div>
                        ))}
                        <div className="md:col-span-2">
