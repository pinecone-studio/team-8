...
export default function SettingsPage() {
  ...
  const [updateMySettings, { loading: saving }] = useMutation(UPDATE_MY_SETTINGS, {
    ...
    update(cache, { data: mutationData }) {
      ...
      cache.writeQuery({
        ...
        data: { mySettings: mutationData.updateMySettings },
      });
    },
  });

  // Local overrides layered on top of server data.
  // Avoids setState-in-effect anti-pattern: committed state lives in Apollo cache;
  // uncommitted edits live in `localOverrides`.
  ...
  const handleNotificationChange = useCallback(
    (key: keyof Pick<SettingsData, "notificationEmail" | "notificationEligibility" | "notificationRenewals">, checked: boolean) => {
      setLocalOverrides((prev) => ({ ...prev, [key]: checked }));
    },
    [],
  );
        ...
        <main className="w-full max-w-7xl min-h-screen py-10 px-6 sm:px-12">
          <div className="mx-auto w-full max-w-[800px] space-y-8">
            ...
            <div className="space-y-6">
              ...
              {/* Profile — read-only from the employee record */}
                ...
                <div className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[14px] font-medium text-[#101828]">Name</label>
                      <p className="px-4 py-2.5 bg-gray-50 border border-[#EAECF0] rounded-xl text-[15px] text-[#344054]">
                        {employee?.nameEng ?? employee?.name ?? "—"}
                      </p>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[14px] font-medium text-[#101828]">Role</label>
                      <p className="px-4 py-2.5 bg-gray-50 border border-[#EAECF0] rounded-xl text-[15px] text-[#344054] capitalize">
                  ...
                  <p className="text-xs text-gray-400">
                    Profile details are managed by HR. Contact your HR team to update your name or email.
                  </p>
                </div>
              </section>
                  ...
                  {loading ? (
                    ...
                    <>
                      <div className="flex items-center justify-between">
                        ...
                        <input type="checkbox" checked={form.notificationEmail} onChange={(e) => handleNotificationChange("notificationEmail", e.target.checked)} className="w-5 h-5 rounded border-[#D0D5DD] text-[#2970FF] focus:ring-[#2970FF] cursor-pointer" />
                      </div>
                      <div className="flex items-center justify-between">
                        <div>
                        ...
                        <input type="checkbox" checked={form.notificationEligibility} onChange={(e) => handleNotificationChange("notificationEligibility", e.target.checked)} className="w-5 h-5 rounded border-[#D0D5DD] text-[#2970FF] focus:ring-[#2970FF] cursor-pointer" />
                      </div>
                      <div className="flex items-center justify-between">
                        <div>
                        ...
                        <input type="checkbox" checked={form.notificationRenewals} onChange={(e) => handleNotificationChange("notificationRenewals", e.target.checked)} className="w-5 h-5 rounded border-[#D0D5DD] text-[#2970FF] focus:ring-[#2970FF] cursor-pointer" />
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <select value={form.language} onChange={(e) => setLocalOverrides((p) => ({ ...p, language: e.target.value }))} className="w-full px-4 py-3 bg-white border border-[#EAECF0] rounded-xl text-[15px] text-[#101828] outline-none appearance-none cursor-pointer focus:border-[#2970FF] focus:ring-1 focus:ring-[#2970FF]">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <select value={form.timezone} onChange={(e) => setLocalOverrides((p) => ({ ...p, timezone: e.target.value }))} className="w-full px-4 py-3 bg-white border border-[#EAECF0] rounded-xl text-[15px] text-[#101828] outline-none appearance-none cursor-pointer focus:border-[#2970FF] focus:ring-1 focus:ring-[#2970FF]">
                        <option>UTC</option>
              ...
              <div className="flex justify-end items-center gap-3 pt-4 pb-12">
                ...
                <button type="button" onClick={handleCancel} disabled={saving} className="px-5 py-2.5 text-[14px] font-medium text-[#344054] bg-white border border-[#D0D5DD] rounded-xl transition-all shadow-sm hover:bg-gray-50 active:scale-[0.98] active:bg-gray-100 disabled:opacity-50">Cancel</button>
                <button type="button" onClick={handleSave} disabled={saving || loading} className="px-6 py-2.5 text-[14px] font-medium text-white bg-[#2970FF] rounded-xl transition-all shadow-sm hover:bg-[#175CD3] active:scale-[0.98] active:bg-[#1557b8] disabled:opacity-50">
